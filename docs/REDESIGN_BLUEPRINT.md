...
- **Data:** TanStack Query → Supabase (project `damcyhiznlpykxskndes` since 2026-09-25; see `docs/DATABASE_MIGRATION_PLAN.md`; the table list below describes the previous project). Tables: `vehicles`, `vehicle_images`, `vehicle_categories`, `reservations`, `booking_inquiries`, `leads`, `event_logs`, `user_roles`; storage bucket `vehicle-images`; edge functions `check-availability` and `create-reservation` (both only reachable from retired routes).

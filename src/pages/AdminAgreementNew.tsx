...
export default function AdminAgreementNew() {
  ...
  const templates = useMemo(() => (data?.templates ?? []) as Array<{ id: string; name: string; version: number; template_definition: TemplateDefinition }>, [data]);
  const [templateId, setTemplateId] = useState("");
  const [agreementData, setAgreementData] = useState<AgreementData>(() => structuredClone(DEFAULT_VEHICLE_CONSIGNMENT_DATA));
  const [saving, setSaving] = useState(false);
  useEffect(() => {
    if (!templateId && templates[0]) {
      setTemplateId(templates[0].id);
      setAgreementData(structuredClone(templateDataKind(templates[0].template_definition) === "long_term_rental" ? DEFAULT_LONG_TERM_RENTAL_DATA : DEFAULT_VEHICLE_CONSIGNMENT_DATA));
    }
  ...
  const chooseTemplate = (nextId: string) => {
    ...
    if (selected) {
      setAgreementData(structuredClone(templateDataKind(selected.template_definition) === "long_term_rental" ? DEFAULT_LONG_TERM_RENTAL_DATA : DEFAULT_VEHICLE_CONSIGNMENT_DATA));
    }
  };

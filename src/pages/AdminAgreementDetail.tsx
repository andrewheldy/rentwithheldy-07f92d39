...
import type { AgreementDetail, ResolvedAgreementDocument, VehicleConsignmentAgreementData } from "@/lib/agreements/types";
...
export default function AdminAgreementDetail() {
  ...
  const [draft, setDraft] = useState<VehicleConsignmentAgreementData | null>(null);
    ...
    <div className="min-h-screen bg-background">
      ...
      <main className="container mx-auto px-4 py-6 sm:py-8">
        ...
        <Tabs defaultValue="details" className="mt-6">
          ...
          <TabsContent value="details" className="mt-6 space-y-6">
            <Card className="p-5 sm:p-6">
              <div className="flex flex-wrap items-start justify-between gap-4"><div><h3 className="text-xl font-semibold">Parties and signatures</h3><p className="mt-1 text-sm text-muted-foreground">Required signers can sign in parallel.</p></div>{detail.status === "draft" && (
                <Dialog open={sendOpen} onOpenChange={setSendOpen}><DialogTrigger asChild><Button disabled={!draft.paymentCadence}><Send className="me-2 h-4 w-4" /> Send for signature</Button></DialogTrigger><DialogContent><DialogHeader><DialogTitle>Send for signature</DialogTitle><DialogDescription>This freezes version {detail.version.number}. Material changes will require a new version and fresh signatures.</DialogDescription></DialogHeader><div><label htmlFor="send-message" className="text-sm font-medium">Optional message</label><Textarea id="send-message" className="mt-2" rows={4} maxLength={1000} value={message} onChange={(event) => setMessage(event.target.value)} /></div><DialogFooter><Button variant="outline" onClick={() => setSendOpen(false)}>Cancel</Button><Button onClick={send} disabled={busy === "send"}><Mail className="me-2 h-4 w-4" /> {busy === "send" ? "Sending…" : "Send invitations"}</Button></DialogFooter></DialogContent></Dialog>
              )}</div>
              {!draft.paymentCadence && detail.status === "draft" && <p className="mt-3 text-sm font-medium text-amber-800">Choose a payment schedule before sending.</p>}
          ...
          <TabsContent value="preview" className="mt-6">{preview ? <><div className="mb-4 flex items-center gap-2 text-sm text-muted-foreground"><Eye className="h-4 w-4" /> {detail.version.frozenAt ? "Frozen signer view" : "Draft preview using the signer renderer"}</div><AgreementDocument document={preview} signatures={detail.signers.map((signer) => ({ id: signer.id, name: signer.fullName, role: signer.role, status: signer.status ?? "pending", signedAt: signer.signedAt ?? null, signatureMethod: signer.signatureMethod, typedSignature: signer.typedSignature }))} /></> : <Card className="p-8 text-center"><h3 className="font-semibold">Preview needs a payment schedule</h3><p className="mt-1 text-sm text-muted-foreground">Return to Agreement details and choose bi-weekly or monthly.</p></Card>}</TabsContent>

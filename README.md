...
Copy `.env.example` to `.env` and fill in the public Supabase variables (`VITE_SUPABASE_PROJECT_ID`, `VITE_SUPABASE_PUBLISHABLE_KEY`, `VITE_SUPABASE_URL`) — never commit real values. Acquisition and agreement workflows also require server-only `SUPABASE_URL`, `SUPABASE_SECRET_KEY`, `RESEND_API_KEY`, `RESEND_FROM_EMAIL`, and `APP_BASE_URL` variables in Vercel. `APP_BASE_URL` must be the canonical HTTPS production origin used in secure signing links. `VITE_EMPOWER_REFERRAL_URL` is optional until the real Rent With Heldy referral URL is supplied.
